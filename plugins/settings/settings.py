...
async def OpenSettings(m: "types.Message"):
    ...
    generate_sample_video = user_data.get("get_generate_sample_video", False)
    generate_ss = user_data.get("get_generate_ss", False)
    buttons_markup = [
        [types.InlineKeyboardButton(f"ᴜᴘʟᴏᴀᴅ ᴀs {'🎥 ᴠɪᴅᴇᴏ' if upload_as_doc else '🗃️ ғɪʟᴇ'}",
                                    callback_data="triggerUploadMode")],
        [types.InlineKeyboardButton(f"Generate Sample Video {'✅' if get_generate_sample_video else '❌'}", 
                                    callback_data="triggerGenSample")],
        [types.InlineKeyboardButton(f"Generate Screenshots {'✅' if get_generate_ss else '❌'}", 
